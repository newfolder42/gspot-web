/**
 * Guess accuracy index — score distribution buckets, colors and aggregate stats.
 *
 * Kept framework-free on purpose: the mobile app renders the same chart, so this
 * file is the single source of truth for buckets, colors and index math.
 */
...
export function formatGuessDistance(distance: number): string {
  if (distance >= 1000) {
    return `${(distance / 1000).toLocaleString('ka-GE', { maximumFractionDigits: 1 })} კმ`;
  }
  return `${Math.round(distance).toLocaleString('ka-GE')} მ`;
}

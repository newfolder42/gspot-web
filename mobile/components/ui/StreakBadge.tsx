  ...
  return (
    <>
      <Pressable
        ...
        style={{
          borderColor: lit ? 'rgba(249,115,22,0.4)' : theme.border,
          backgroundColor: lit ? 'rgba(249,115,22,0.1)' : 'rgba(113,113,122,0.12)',
        }}
      >
        <MaterialCommunityIcons name="fire" size={15} color={lit ? '#FB923C' : theme.textMuted} />
        <Text className="text-sm font-semibold" style={{ color: lit ? '#FB923C' : theme.textMuted }}>
          {streak.current}
        </Text>
      </Pressable>
      ...
      <Modal transparent animationType="fade" visible={open} onRequestClose={() => setOpen(false)}>
        ...
        >
          <View className="w-full rounded-2xl bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 px-4 py-3">
            <Text className="text-sm font-semibold text-zinc-900 dark:text-white">უწყვეტობა: {streak.current} დღე</Text>
            <Text className="mt-1 text-xs text-zinc-600 dark:text-zinc-400">რეკორდი: {streak.longest} დღე</Text>
